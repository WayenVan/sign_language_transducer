...
class RnnLayer(nn.Module):
    def __init__(self, input_size, debug=False, hidden_size=512, num_layers=1, dropout=0.3,
                 bidirectional=False, rnn_type='LSTM', num_classes=-1):
    ...
    def forward(self, src_feats, src_lens, hidden=None):
        ...
        sorted_src_lens, indices = torch.sort(src_lens, descending=True)
        src_feats = src_feats[:, indices]
        packed_emb = nn.utils.rnn.pack_padded_sequence(src_feats, sorted_src_lens, enforce_sorted=False)
        ...
        rnn_outputs = rnn_outputs[:, desorted_indices]
